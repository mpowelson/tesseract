...
void OMPLMotionPlannerDefaultConfig::init()
{
  ...
  if (configuration == OMPLProblemConfiguration::SE3_STATE_SPACE_ROBOT_ON_POSITIONER ||
      configuration == OMPLProblemConfiguration::SE3_STATE_SPACE_ROBOT_WITH_EXTERNAL_POSITIONER)
  {
  ...
  if (configuration == OMPLProblemConfiguration::REAL_STATE_SPACE)
    extractor_ = std::bind(
        &tesseract_planning::RealVectorStateSpaceExtractor, std::placeholders::_1, manip_inv_kin_->numJoints());
