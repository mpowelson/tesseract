...
TEST_F(TesseractPlanningDescartesUnit, DescartesPlannerFixedPoses)  // NOLINT
{
  ...
  for (int i = 0; i < 10; ++i)
  {
    ...
    {
      if (isCompositeInstruction(official_results[j].getType()))
      {
        ...
        {
          if (isCompositeInstruction((*sub_official)[k].getType()))
          {
            EXPECT_TRUE(false);
          }
          else if (isMoveInstruction((*sub_official)[k].getType()))
          {
      ...
      else if (isMoveInstruction(official_results[j].getType()))
      {
