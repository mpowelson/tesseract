...
TEST_F(TesseractPlanningInterpolatorPlannerUnit, JointJoint)  // NOLINT
{
  ...
  program.setDescription("JointJoint Program");

  Waypoint wp1 = JointWaypoint(Eigen::VectorXd::Zero(
      tesseract_ptr_->getFwdKinematicsManager()->getFwdKinematicSolver("manipulator")->numJoints()));
  Waypoint wp2 = JointWaypoint(Eigen::VectorXd::Ones(
      tesseract_ptr_->getFwdKinematicsManager()->getFwdKinematicSolver("manipulator")->numJoints()));

  PlanInstruction plan1(wp1, PlanInstructionType::FREESPACE);
  program.push_back(plan1);
  PlanInstruction plan2(wp2, PlanInstructionType::FREESPACE);
  program.push_back(plan2);

  // Create the planner
  InterpolatorMotionPlanner planner("test");
  EXPECT_EQ(planner.getName(), "test");

  // Set the config
  InterpolatorPlannerConfig::Ptr config = std::make_shared<InterpolatorPlannerConfig>(tesseract_ptr_);
  config->instructions = program;
  config->seed = GenerateSeed(config->instructions, *wp1.cast_const<JointWaypoint>(), 10, 10);
  config->seed.setDescription("JointJoint Seed");
  planner.setConfiguration(config);

  if (DEBUG)
  {
    config->instructions.print();
    config->seed.print();
  }

  PlannerResponse response;
  planner.solve(response, PostPlanCheckType::DISCRETE_CONTINUOUS_COLLISION, DEBUG);

  std::cout << response.joint_trajectory.trajectory << std::endl;
